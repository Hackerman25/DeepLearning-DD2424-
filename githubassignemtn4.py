...
def load_data(file_name):
    ...
    compile_output = {'book_data': data, 'book_chars': chars, 'K': K, 'char_to_ind': char_to_ind,
...
class RecurrentNeuralNetwork():
    ...
    def compute_gradients(self, X, Y, h0):
        ...
        grad_U, grad_W, grad_V, grad_b, grad_c = np.zeros_like(self.params['U']), np.zeros_like(
            self.params['W']), np.zeros_like(self.params['V']), np.zeros_like(self.params['b']), np.zeros_like(
            self.params['c'])
...
if __name__ == '__main__':
    start = time.time()
    data = load_data(r"C:\Users\marcu\Documents\GitHub\DeepLearning-DD2424-\Datasets\goblet_book.txt")
    sig = 0.01
    seq_len = 25
    epoch = 7
    eta = 0.1
